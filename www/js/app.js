...
$(document).ready(function() {
  ...
  for (i=0;i<rss.length;i++) {
    ...
    // BUILD THE LEGEND
    var lines = ['1','2','3','4','5','6','7','A','C','E','B','D','F','M','G','J','Z','L','N','Q','R','W']
    ...
    var load_map = function (line_selected, json_selected, subsequent_click) {   
        ...
        // It pulls in the JSON generated by polopoly (and stored locally, manually updated when there's something new)
        // and compares the stop names in the JSON (the stop names will always be the part of the article abstract before the first ": ", yes they will.
        $.getJSON(json_selected, function(data){
            ...
            // mta is the data stored in data/mta.min.geojson
            ...
            var my_stops = []
            // data is the name of the data pulled in from the article json feed
            var len = data.length;
            // STOP-NAME MATCHING
            for ( var p = 0; p < len; p++ ) {
            ...
            if ( !is_mobile ) {
                // stops is stored in data/subway_geo.min.geojson
                var len = stops.features.length;
                for ( var i= 0 ; i < len; i++ ) {
                    if (stops.features[i].properties.line == line_selected) {
                        //console.log(stops.features[i].properties.stations);
